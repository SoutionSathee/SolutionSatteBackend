...
export const createLeadForUdyamCertificate = async (req, resp) => {
  try {
    const { serviceId, clientId, businessAdress } = req.body;
...
export const editHomeLoanServiceControllerFPS = async (req, res) => {
  try {
    ...
    const { isPending, isFailed,  isSuccess } = req.body
...
export const editGSTRegistrationControllerFPS = async (req, res) => {
  try {
    const {LoanId} = req.query
    const { isPending, isFailed,  isSuccess } = req.body
    const updatedResponse = await GSTRegistrationModel.findByIdAndUpdate(
      LoanId,
      req.body,
      { new: true, runValidators: true }
    );
